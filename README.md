...
python platform/scripts/generate_week.py --week 1

# 옵션 override
python platform/scripts/generate_week.py --week 4 --level L2 --num-problems 3
```

주차 설정은 `platform/config/weeks/week-NN.yaml`에서 관리합니다.

### 채점

```bash
python platform/scripts/score_submissions.py --week 1
```

## 배포

1. GitHub repo Settings → Pages → Source: `main` branch, `/platform/frontend` folder (또는 Netlify 사용)
2. `platform/config/members.yaml`에 스터디원 등록
3. `platform/config/group.yaml`에서 repo 경로 확인
4. GitHub Actions에 `GROUP_SALT` 시크릿 등록

## 프로젝트 구조

```
weeks/                      ← 주차별 스터디 자료 (게시판)
  week-01/ ~ week-08/

members/                    ← 멤버별 데이터
  leejiseok/
    README.md               ← 프로필
    submissions/            ← 제출물 (week-01/L1-001.json)
